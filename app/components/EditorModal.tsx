...
type Props = { open: boolean; onClose: () => void; onSave?: (data: any) => void };
...
function PixelSprite({ rows, size, colorMap, className, label, decorative }: PixelSpriteProps) {
  const safeRows = rows ?? [];
  const width = size?.[0] ?? safeRows?.[0]?.length ?? DEFAULT_SPRITE_SIZE[0];
  const height = size?.[1] ?? safeRows?.length ?? DEFAULT_SPRITE_SIZE[1];
  const pixels = safeRows.length ? renderPixelRects(safeRows, colorMap, label ?? "sprite") : [];
...
function EditorContent({ onSave }: { onSave?: (data: any) => void }) {
  ...
  const compositeRows = useMemo(() => {
    ...
    const layers = order.map((category) => {
      ...
      // Para pantalones animados, crear una capa de 16x16 con el pantalón en la posición correcta
      if (sprite.type === 'animated' && sprite.frames) {
        const pantsFrame = sprite.frames[0]; // Usar primer frame para preview
        const frameWidth = sprite.frameSize?.[0] ?? 10;
        const frameHeight = sprite.frameSize?.[1] ?? 3;
        
        // Crear una capa de 16x16 con el pantalón posicionado en filas 13-15
        const fullLayer: string[] = [];
        const legStartY = 13;
        const legStartX = 3; // Centrar 10px en 16px: (16-10)/2 = 3
        
        for (let y = 0; y < 16; y++) {
          let row = '';
          for (let x = 0; x < 16; x++) {
            const pantsY = y - legStartY;
            const pantsX = x - legStartX;
            
            if (pantsY >= 0 && pantsY < frameHeight && pantsX >= 0 && pantsX < frameWidth) {
              const pantsRow = pantsFrame[pantsY] || '';
              row += pantsRow[pantsX] || '.';
            } else {
              row += '.';
            }
  ...
  // Crear compositeRows sin pantalón para el juego (cuando el pantalón es animado)
  const compositeRowsForGame = useMemo(() => {
    if (!manifest) {
      return [];
    }

    const selectedPantsSprite = pantsSprites.find(s => s.id === selectedPantsId) ?? pantsSprites[0];
    const isPantsAnimated = selectedPantsSprite?.type === 'animated';

    // Si el pantalón no es animado, usar los mismos compositeRows
    if (!isPantsAnimated) {
      return compositeRows;
    }

    // Si es animado, excluir el pantalón de las capas
    const order = manifest.layerOrder.length
      ? [
          ...manifest.layerOrder,
          ...Object.keys(categories).filter((name) => !manifest.layerOrder.includes(name)),
    ...
    const layers = order.map((category) => {
      // Excluir pantalón si es animado
    ...
    return rows;
  }, [manifest, categories, selectedSprites, spriteSize, compositeRows, pantsSprites, selectedPantsId]);
  ...
  return (
      ...
      <main className="pixel-frame">
              ...
              {pantsSprites.length > 1 ? (
                <div className="option-grid">
                  {pantsSprites.map((sprite) => {
                    // Para pantalones animados, usar el primer frame; para estáticos, usar rows
                    const displayRows = sprite.type === 'animated' && sprite.frames 
                      ? sprite.frames[0] 
                      : sprite.rows;
                    const displaySize = sprite.type === 'animated' && sprite.frameSize 
                      ? sprite.frameSize 
                      : spriteSize;
                    return (
                      <OptionButton
        ...
        <footer className="pixel-actions">
          ...
          <button 
            ...
            onClick={() => {
              if (onSave) {
                // Obtener el sprite de pantalón seleccionado con sus frames
                const selectedPantsSprite = pantsSprites.find(s => s.id === selectedPantsId) ?? pantsSprites[0];
                ...
                onSave({
                  ...
                  message,
                  selectedSprites,
                  compositeRows: compositeRowsForGame, // Usar rows sin pantalón para el juego
                  spriteSize,
                  tokenColors,
                  pantsSprite: selectedPantsSprite // Pasar el sprite completo con frames
                });
              }
            }}
          >
    ...
    export default function EditorModal({ open, onClose, onSave }: Props) {
      if (!open) return null;

      return (
        <div className="info-modal-overlay" role="dialog" aria-modal="true">
          <div className="info-modal-shell" onClick={(e) => e.stopPropagation()}>
            <button type="button" aria-label="Cerrar" className="info-modal-close" onClick={onClose}>×</button>

            <div className="editor-modal-grid">
              <div className="editor-unified">
          ...
          <style jsx global>{`
            .info-modal-overlay { position: fixed; inset: 0; display:flex; align-items:center; justify-content:center; background: rgba(0,0,0,0.45); z-index:1000; }
            .info-modal-shell { width: min(1100px, 96%); max-height: 88vh; overflow: auto; position: relative; }
            .info-modal-close { position: absolute; right: 12px; top: 12px; z-index: 5; background: #f4e6c9; border:1px solid #2b2417; width:34px; height:34px; display:flex; align-items:center; justify-content:center; font-size:18px; }
            .editor-modal-grid { display:flex; gap:18px; align-items:stretch; padding: 18px; box-sizing: border-box; }
            .editor-unified { width:100%; }
          `}</style>
        </div>
      );
    }
